 ...
 */
int send_hello(const char* host, const char* port, uint64_t client_id, std::string client_pwd, hello_type type) {
    // create connection
    ...
    strcpy(hello.client_pwd, client_pwd.c_str());
    hello.pwd_size = client_pwd.size();
...
int Client::connect(const char* host, const char* port) {
    ...
    this->request_fd = send_hello(host, port, this->id, this->pwd, REQUESTS_HELLO);
    if (this->request_fd < 0) return -1;

    server_hello_t pwd_status;
    if (read(this->request_fd, &pwd_status, sizeof(server_hello_t)) < (int) sizeof(server_hello_t)) {
        return -1;
    } else {
        if (pwd_status.pwd_status == PWD_REJECTED) {
            fprintf(stderr, "Invalid password. Stopping connection...\n");
            return -1;
        }
    }

    // create connection for notifications
    this->notification_fd = send_hello(host, port, this->id, this->pwd, NOTIFICATIONS_HELLO);
